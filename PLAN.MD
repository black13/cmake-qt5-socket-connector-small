...
**Polish Cycle – Branch Per Goal**
1. `polish/snap-grid` – refine grid rendering & snapping indicator
2. `polish/view-navigation` – smoother zoom/pan, recenter shortcuts
3. `polish/context-menu` – UX tweaks for scripted-node menu (disabled states, log hygiene)
4. `polish/status-logging` – consolidate log spam, add concise status HUD for scripts/saves
5. `polish/selection-copy` – prep selection HUD for future copy/paste/undo work
...
- `scripts/coverage_report.sh` builds with clang coverage flags, runs NodeGraph (passes through CLI args), and emits an `llvm-cov report` so we can track UI/script execution coverage on macOS.
- `scripts/coverage_scenario.js` drives Graph facade APIs (create/move/connect/delete, save/load) to raise coverage during automated runs; launch via `coverage_report.sh --script scripts/coverage_scenario.js`.
- **SCPI emulator subproject** (planned) – emulate the legacy EMC bench on a host machine before moving to Raspberry Pi hardware. Target: lightweight QEMU/VM on macOS, NI-VISA client talks over TCP/IP to the emulator’s SCPI port. Document requirements and start with a software-only spectrum analyzer/signal generator before introducing hardware.
- Detailed requirements tracked in `scpi/qemu_notes.md` (install QEMU, Linux guest, SCPI service, NI-VISA integration, eventual Raspberry Pi port).
- Status: Environment validated on macOS 12.7
  - NI-VISA 23.5.0 already installed under `/Applications/National Instruments/NI-VISA` (NI-MAX + NI IO Trace confirmed working)
  - QEMU 10.1.2 installed via MacPorts (`qemu-system-x86_64 --version`)
  - Next step: build a SCPI server running locally (or inside the new QEMU guest) so NI-MAX can discover it over TCP/IP before porting to Raspberry Pi hardware. 
    * Guest OS: minimal Ubuntu Server (no desktop) or similar, installed via QEMU with QCOW2 disk + user-mode networking (port 5025 forwarded). 
    * Inside the guest, run a lightweight SCPI service (Python/Go/C++) that responds to basic commands (`*IDN?`, `*RST`, `.MEAS?`) before modeling full EMC instruments. 
    * Document exact steps in `scpi/qemu_notes.md` as we build the guest and emulator.
- **SCPI/LXI transport focus (Dec 2025)** – new goal is to remove VISA termination-character hacks by exposing the emulator as a proper LXI instrument. Reference: [EEVblog thread “Open source lxi-tools and liblxi v1.0 released for GNU/Linux” posts 225‑249](https://www.eevblog.com/forum/testgear/open-source-lxi-tools-and-liblxi-v1-0-released-for-gnulinux/225/) which explains why most LXI gear offers SCPI via three transports (RAW TCP, VXI‑11 over ONC RPC, HiSlip). Plan:
  1. **Document protocol behavior** – keep Wireshark captures of NI‑MAX talking to a known-good VXI‑11 server (see “Python baseline” below) so we know the exact CreateLink / DeviceRead / DeviceWrite sequence NI expects.
  2. **Python proof-of-concept** – clone the community-maintained [`python-vxi11-server`](https://github.com/coburnw/python-vxi11-server) fork (BSD license) and run `examples/simple_scpi_server.py`. This exposes `TCPIP::<host>::INSTR` without any VISA tweaks, giving us real traffic to study and a quick demo for stakeholders (“if it works, it works” even though it’s Python).
  3. **Capture reference traces** – while the Python server is running, connect with NI‑MAX/NIvisaic, issue `*IDN?`, `*RST`, etc., and save `.pcap` files filtered on RPC/VXI‑11 ports. These captures become our spec when porting the dispatcher to C++.
  4. **Integrate liblxi (C)** – pull the existing `liblxi/` tree into the build via `FetchContent` (only the VXI‑11 server bits: `vxi11core_svc.c`, `vxi11.c`, generated XDR files). Wrap the callbacks so they invoke our existing SCPI dispatcher (`scpi/share/asio_scpi_server.cpp`). End result: two front ends (Boost.Asio raw TCP and VXI‑11) sharing a common command handler.
  5. **Build/test wiring** – teach `build.sh` (Debug/Release) to compile/link the VXI‑11 server, and gate the download paths so offline builds still succeed. Update the Windows `.bat` later but keep placeholders now.
  6. **Documentation** – expand `README.md` once VXI‑11 works: how to run the server, how NI‑MAX discovers it via `TCPIP::<host>::INSTR`, and note that VXI‑11 eliminates the need to toggle “Enable termination character” / `Serial End In Mode`.
  7. **Future transports** – after VXI‑11 we can evaluate HiSlip (libhislip or similar) and optional scripting (Duktape/QtScript) so engineers can build SCPI behaviors without touching C++. Lua is explicitly out per stakeholder preference.

- **Linux VM testbed** – macOS SIP and launchd restrictions make rpcbind/VXI‑11 testing painful (macOS 12 won’t allow `rpcbind -i`, port 111 registrations require root, and VirtualBox shared folders simplify cross-platform builds). To eliminate that friction:
  * Stand up a lightweight Ubuntu Server VM in VirtualBox (2 vCPU, 4 GB RAM, 40 GB disk). Use **Bridged** networking so NI‑MAX on macOS can reach `TCPIP::<vm-ip>::INSTR` directly.
  * Enable shared folders (or SSHFS) so the VM sees `/Users/jjosburn/temp/cmake-qt5-socket-connector-small` and we can edit on macOS while running servers inside Linux.
  * Inside the VM: `sudo apt install rpcbind python3 python3-pip git`, `sudo systemctl enable --now rpcbind`, then clone `python-vxi11-server` and run the demo SCPI device without sudo. Linux’s rpcbind accepts registrations instantly, letting us capture Wireshark traces and iterate faster.
  * Status: VM online (Ubuntu Server), SSH reachable from macOS. Next: verify packages (`rpcbind`, `python3`, `git`), pull `python-vxi11-server`, and document Wireshark captures.
- **Network setup recap (Dec 20)** – Host-only adapter `vboxnet0` added alongside NAT so the VM now has:
  * Adapter 1 (NAT) → `enp0s3`, IP `10.0.2.15`, SSH port-forward on host `127.0.0.1:2222` unchanged.
  * Adapter 2 (Host-only `vboxnet0`) → `enp0s8`, IP `192.168.56.2` after enabling DHCP via Netplan (`dhcp4: true`). Install `isc-dhcp-client` if needed so `dhclient enp0s8` works.
  * Guest Additions installed (VirtualBox ISO + `build-essential dkms linux-headers-$(uname -r)`) for shared folders/clipboard.
  * Use `192.168.56.2` for NI‑MAX (`TCPIP::192.168.56.2::INSTR`) and Wireshark captures; NAT path stays for internet/outbound traffic.
- **Documentation artifacts** – Earlier NI‑MAX reverse-engineering generated a large `doxygen_out/` (~700 MB). Keep those HTML/PDF exports only if we’re actively referencing them during analysis; otherwise, archive or regenerate to avoid wasting disk space in the working tree.
- **VXI‑11 server (Dec 2025)** – Python baseline confirmed (`TCPIP::192.168.56.3::INSTR` works). Next wave is a native C/C++ implementation:
  1. Use EPICS/Tektronix `vxi11core.rpcl` (`~/asyn/asyn/vxi11/vxi11core.rpcl` on the VM) and run `rpcgen -a -C` to generate the ONC/RPC sources (`vxi11core_svc.c`, `_xdr.c`, headers). Repeat for `vxi11intr.rpcl` if we need interrupts.
  2. Integrate the generated files into the C++ backend (thin C RPC wrappers calling C++ handlers). Keep transport and SCPI logic in C++ so we can reuse it across front ends.
  3. **Build-from-source policy** – avoid OS‑provided libs for the emulator path. Vendor or build required deps (e.g., Duktape, libtirpc) under `third_party/` so Debug builds are reproducible on the VM without system packages.
  4. **Duktape scripting** – add `duktape.c/duktape.h` under `third_party/duktape/`, wire a script hook into the SCPI dispatcher, and keep a C++ fallback path for core commands.
  5. **Debug-first builds** – default the VM build scripts to Debug unless overridden; expose logging toggles (`VXI11_DEBUG=1`) so every step stays observable while the SCPI surface grows.
  6. Keep the Python server + NI-VISA captures (`dec22capture.txt`) as the golden reference; verify the new server registers with rpcbind, responds identically to `*IDN?`, and matches Wireshark before retiring the Python shim.
  7. Reference: Tektronix Application Bulletin 80‑3 (in repo as `Capture.txt` + PDF) explains the ONC/RPC registration flow (server calls `svc_register`, rpcbind maps program 0x0607AF -> TCP port, clients query rpcbind before issuing `create_link`, `device_write`, `device_read`). Document this sequence in README with a Mermaid diagram showing `Client -> rpcbind -> Server` messages so future work understands why rpcbind must be running.
- **VM/Shared folder recap** – new Ubuntu VM (70 GiB disk) rebuilt Dec 22. Guest Additions installed, user added to `vboxsf`, host path `.../shared/` auto-mounts inside the VM as `/media/sf_shared` so we can edit on macOS and build/run inside Linux. Adapter1=NAT (SSH port 2222), Adapter2=Host-only (`vboxnet0`, `enp0s8=192.168.56.3`).
- **VXI‑11 server (Dec 2025)** – We now have a working Python baseline (python-vxi11-server) that NI‑VISA and PyVISA talk to via `TCPIP::192.168.56.3::INSTR`. Next wave is a native C/C++ implementation:
  1. Use the EPICS/Tektronix `vxi11core.rpcl` (`/Users/jjosburn/temp/asyn/asyn/vxi11/vxi11core.rpcl`) and run `rpcgen -a -C` to generate the ONC/RPC server stubs (`vxi11core_svc.c`, `_xdr.c`, headers). Repeat for `vxi11intr.rpcl` if we support interrupts.
  2. Integrate those generated sources into our CMake build and implement the `create_link`, `device_write`, `device_read`, etc. handlers so they call the existing Boost.Asio SCPI dispatcher (the logic currently in `scpi/share/asio_scpi_server.cpp`). The RPC layer replaces the raw TCP listener so only VXI‑11 endpoints are exposed.
  3. Keep the Python server + NI-VISA traces (e.g., `dec22capture.txt`) as the golden reference—verify the new C/C++ server registers with `rpcbind`, responds identically to `*IDN?`, and matches the Wireshark captures before retiring the Python shim.
- **Addendum – VM bring-up checklist:**
  1. **VM creation** – VirtualBox VM documented (Ubuntu Server 24.04, bridged NIC, shared folder). SSH port-forwarding confirmed (`ssh -p 2222 jjosburn@127.0.0.1`).
  2. **Baseline packages** – inside the VM run `sudo apt update && sudo apt install rpcbind python3 python3-pip git build-essential cmake gdb libboost-all-dev libssl-dev` so Boost/Asio builds succeed locally.
  3. **Clone SCPI tooling** – `git clone https://github.com/coburnw/python-vxi11-server.git`; run `python3 demo_servers/time-device.py` to confirm rpcbind/portmapper registration.
  4. **Capture traffic** – from macOS (bridged NIC) connect NI‑MAX to `TCPIP::<vm-ip>::INSTR`, issue SCPI commands, and save Wireshark capture files (`Create_Link`, `Device_Write`, `Device_Read` sequences). These captures feed the C++/liblxi implementation.
- **UI backlog (Graph editor):**
  * Build a richer “Recent/Open Graph” surface that remembers the last few XMLs (per project) with metadata like modified time, thumbnail, and storage path so it’s easy to reopen the correct network description. This should also surface as a `File → Recent Graphs` menu (and optionally a welcome dialog) so users can reopen the last N files with a single click.
  * Add export tooling for documentation: given the current graph scene, generate Mermaid diagrams (`graph TD`, `flowchart LR`, etc.) from the node/edge list. Provide CLI + UI entry point (e.g., File → Export → Mermaid) that writes `.mmd` alongside the original XML for downstream docs/Wikis.

> **VXI‑11 refresher:** defined by `vxi11core.rpcl` (same file in `liblxi/src`). RPCGen creates `*_svc.c`, `*_clnt.c`, and XDR serializers. A device session starts with `Create_Link`, then repeated `Device_Write` / `Device_Read`. Termination is negotiated via flags inside the RPC messages, so VISA never needs manual “termination char” tweaks. This is why NI‑MAX prefers VXI‑11 endpoints for SCPI instruments.
>
> **Where rpcbind fits:** VXI‑11 rides on ONC RPC, so every server must register its program number/port with the portmapper (`rpcbind`, service on TCP/UDP 111). NI‑MAX (or any VXI‑11 client) first contacts rpcbind to ask “Which port serves program 0x0607AF (VXI‑11 core)?” and then talks directly to that port for all SCPI traffic. Without rpcbind running the server can’t register, and clients cannot discover it—hence why macOS SIP/launchd issues blocked our Python proof-of-concept and why the Linux VM (where `systemctl enable --now rpcbind` just works) is our practical testbed.
...
- Observers exchange UUIDs, geometry structs, and payload data—never `QGraphicsItem*`.
...
### Phase 1 – Event Hub Stabilization
...
### Phase 2 – Snapshot-Based Rendering Bridge
...
### Phase 3 – Autosave & Persistence Observers
1. Port `XmlAutosaveObserver` to the event hub—remove any direct `Scene::items()` iteration.
...
### Phase 4 – Diagnostics, Logging, Undo Preparation
...
### Phase 5 – JavaScript Validation & Examples
...
- [x] Synthetic workload helper: expose a C++ `SyntheticWork::run(request)` utility so scripts can trigger deterministic “heavy” tasks (hashing, math loops, sleeps) without reimplementing them in JS; wire it through `graph.runSyntheticWork()` and the ScriptNode API so both nodes and edges share the same pathway.
...
- **Window chrome & menus** – File (New, Open, Save, Save As), Edit shortcuts (Ctrl+Z placeholder), View controls (Zoom Fit/Reset), Help/About dialogs, status bar text.
- **Node Palette Widget** – category filter/search, tooltip descriptions, drag-and-drop from every template button, keyboard navigation, scrollbar behavior when resized.
- **Graph View / Scene** – node placement, rubber-band selection, Delete key handling, context menu disablement, zoom/pan gestures, ghost-edge snapping, socket hover highlights.
- **Autosave & Logging** – autosave timer toggles, manual save triggers, log rolling per session, message consistency across UI actions.
- **JavaScript Interface** – `--script` CLI path, startup script set via window, console logging, error propagation to log and UI dialogs.
- **File IO dialogs** – Qt file pickers for open/save, overwrite prompts, handling of missing files, default directories.
...
3. **Graph Editing**
   - [ ] Create Source→Transform→Sink chain using drag-drop; connect sockets by dragging; confirm ghost edge highlights valid targets only.
...
3. **Autosave Pressure Test**
   - Lower autosave interval to 1s, spam node movements, and verify file locking/errors don’t appear; check `autosave.xml` writes stay <1 KB for small scenes.
...
- [ ] Automate a subset via `scripts/` JS harness; aim to cover Palette→Scene, Save/Load, and Autosave flows first.
- [ ] Add CI note: once headless mode exists, convert these manual tests into scripted regressions.

### UI → Graph Surface → JavaScript Validation Chain
...
**Clean-state rule:** No leftovers—after each pass, clear the graph (`graph.clearGraph()` / File > New) so the next scenario begins in a pristine scene and autosave/logs are easy to audit.

**Palette expansion ideas:** When new node concepts emerge (custom templates, JS-driven nodes), register them via `NodeTypeTemplates` and add matching palette buttons as part of the verification chain above (UI → Graph → JS); the plan’s table doubles as the checklist for onboarding future palette entries.
...
### Scenario A – Save → Load → Edit → Save → Delete (UI + Graph + JS)
...
3. File → Save As… (or Ctrl+S) → write `scenario_a_step1.xml`; note log lines (`Saving graph to:` etc.).
4. File → New (or `graph.clearGraph()` via command palette) to clear; confirm autosave nodes=0.
5. File → Open… → load `scenario_a_step1.xml`.
6. Drag an additional node (e.g., SINK) into the scene.
7. Ctrl+S to overwrite the same file; confirm log shows nodes incremented.
8. File → Open… (same file) to verify both nodes reappear.
...
1. **Branch 4.1 – feature/node-javascript-behavior**
...
4. **Branch 6.1 – test/javascript-integration-examples**
...
### Branch 1.5: Ghost Edge Typed Access ✅ COMPLETE
**Branch:** `refactor/ghost-edge-typed` → Status: MERGED (local)
...
### Branch 1.6: Window Typed Queries ✅ COMPLETE
**Branch:** `refactor/window-typed-queries` → Status: MERGED (local)
...
### Branch 1.7: Factory Typed Serialization ✅ COMPLETE
**Branch:** `refactor/factory-typed-serialization` → Status: MERGED (local)
...
### Branch 2.1: Fix Graph::saveToFile() 🔥 ✅ COMPLETE
**Branch:** `fix/graph-save-to-file` → Status: MERGED (local)
...
### Branch 2.2: Fix Node Position Precision ✅ COMPLETE
**Branch:** `fix/node-position-precision` → Status: MERGED (local)
...
- [ ] `fix/memory-safety-step-6`: documentation & cleanup pass—remove temporary diagnostics once scripting scenarios prove stability, describe the destruction/observer model in README/PLAN, and note the optional autosave guard.

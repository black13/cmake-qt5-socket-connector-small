...
if (Test-Path "complete_graph.xml") {
    Write-Host "[OK] Graph saved: complete_graph.xml" -ForegroundColor Green
    Write-Host ""
    Write-Host "Preview of saved graph:" -ForegroundColor Yellow
    Get-Content "complete_graph.xml" | Select-Object -First 20
    Write-Host "..." -ForegroundColor Gray
} else {
    Write-Host "WARNING complete_graph.xml not found" -ForegroundColor Yellow
}
...
if (Test-Path "logs\") {
    ...
    if ($latestLog) {
        Write-Host "[OK] JavaScript log: $($latestLog.Name)" -ForegroundColor Green
        Write-Host ""

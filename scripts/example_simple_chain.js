...
console.log("✅ Created SOURCE at (100, 200)");

var transform = Graph.createNode("TRANSFORM", 300, 200);
console.log("✅ Created TRANSFORM at (300, 200)");
...
if (edge1 !== "") {
    console.log("✅ Connected SOURCE → TRANSFORM");
} else {
    console.error("❌ Failed to connect SOURCE → TRANSFORM");
}

// TRANSFORM output (socket 1) → SINK input (socket 0)
var edge2 = Graph.connect(transform, 1, sink, 0);
if (edge2 !== "") {
    console.log("✅ Connected TRANSFORM → SINK");
} else {
    console.error("❌ Failed to connect TRANSFORM → SINK");
}
...
console.log("\n✅ Graph saved to simple_chain.xml");
